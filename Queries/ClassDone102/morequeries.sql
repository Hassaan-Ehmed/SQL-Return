...
SELECT * FROM EMP INNER JOIN DEPT ON EMP.DEPT =  DEPT.DEPTNO ORDER BY ENAME
...
SELECT distinct A.ename , A.mgr FROM EMP A, EMP B where A.empno<>B.empno AND A.mgr = B.mgr 

SELECT DEPT FROM EMP UNION SELECT DEPTNO FROM DEPT;

-- Subquery
SELECT * FROM EMP WHERE DEPT in (SELECT DEPTNO FROM DEPT);

SELECT * FROM EMP WHERE DEPT IN (SELECT DEPTNO FROM DEPT WHERE DEPTNO IN (SELECT DEPT FROM EMP  WHERE JOB = 'SALESMAN'))


addingAllTheNumber / howManyNumbers the
